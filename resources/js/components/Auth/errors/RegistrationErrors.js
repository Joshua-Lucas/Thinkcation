    ...
    return(
        <div className=" flex items-center my-4 py-4 px-2 bg-red-100 border-t-4 border-darkaccent rounded-md w-full">
            ...
            <div className="flex flex-col pl-3">
                ...
                <p className={emailError === '' || emailError === null  ? 'hidden' : 'text-xs font-light pb-1'}>{emailError}</p>
                <p className={passwordError === '' || passwordError === null  ? 'hidden' : 'text-xs font-light'}>{passwordError}</p>
            </div>
        </div>
    );
